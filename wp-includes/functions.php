...
function woocommerce_custom_product_info_to_cart($item_data, $cart_item)
{
    if (empty($cart_item['product_info'])) {
        return $item_data;
    }

    $item_data[] = array(
        'key'     => __('Product Detail', 'iconic'),
        'value'   => ($cart_item['product_info']),
        'display' => '',
    );
...
add_filter('woocommerce_get_item_data', 'woocommerce_custom_product_info_to_cart', 10, 2);

function filter_woocommerce_cart_item_thumbnail($product_get_image, $cart_item, $cart_item_key)
{
    // make filter magic happen here...
    if (isset($cart_item['custom_product_thumbnail'])) {
        $product_get_image = $cart_item['custom_product_thumbnail'];
...
function remove_link_from_cart_item($product_link, $cart_item, $cart_item_key)
{
    // make filter magic happen here...
...
add_filter('woocommerce_cart_item_permalink', 'remove_link_from_cart_item', 10, 4);
/*
...
function product_thumbnail_in_checkout($product_name, $cart_item, $cart_item_key)
{
    ...
    $thumbnail   = $cart_item['product_image'];
