...
class Sharing_Service {
	...
	public function __construct() {
		$this->default_sharing_label = __( 'Share', 'pojo-sharing' );
	}
...
function sharing_register_post_for_share_counts( $post_id ) {
	global $pojo_sharing_counts;

	if ( ! isset( $pojo_sharing_counts ) || ! is_array( $pojo_sharing_counts ) )
		$pojo_sharing_counts = array();

	$pojo_sharing_counts[ (int) $post_id ] = get_permalink( $post_id );
}
...
function sharing_add_footer() {
	...
	if ( apply_filters( 'sharing_js', true ) ) {
		...
		if ( is_array( $pojo_sharing_counts ) && count( $pojo_sharing_counts ) ) :
			$sharing_post_urls = array_filter( $pojo_sharing_counts );
...
function sharing_add_header() {
	...
	if ( count( $enabled['all'] ) > 0 )
		wp_enqueue_style( 'sharedaddy', plugin_dir_url( __FILE__ ) .'assets/css/sharing.css', array() );
}
...
function sharing_display( $text = '', $echo = false ) {
	...
	if ( !is_feed() ) {
		...
		} elseif ( in_array( 'index', $global['show'] ) && ( is_home() ) ) {
			$show = true;
		}
	}
	...
	$switched_status = atmb_get_field( 'po_sharing_disabled', $post->ID, Pojo_MetaBox::FIELD_CHECKBOX );
	...
	if ( $show ) {
		...
		if ( count( $enabled['all'] ) > 0 ) {
			...
			if ( $global['sharing_label'] != '' )
				$sharing_content .= '<h5 class="sd-title"><span>' . $global['sharing_label'] . '</span></h5>';
			...
			if ( count( $enabled['hidden'] ) > 0 ) {
				if ( count( $enabled['visible'] ) > 0 )
					$expand = __( 'More', 'pojo-sharing' );
				else
					$expand = __( 'Share', 'pojo-sharing' );
			...
			// Register our JS
			wp_register_script( 'sharing-js', plugin_dir_url( __FILE__ ).'assets/js/sharing.js', array( 'jquery' ), '20121205' );
