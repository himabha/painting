...
function sharing_add_meta_box() {
	...
	$title = apply_filters( 'sharing_meta_box_title', __( 'Sharing', 'pojo-sharing' ) );
...
function sharing_meta_box_content( $post ) {
	...
	$disabled = atmb_get_field( 'sharing_disabled', $post->ID, Pojo_MetaBox::FIELD_CHECKBOX ); ?>
	...
	<p>
		<label for="enable_post_sharing">
			...
			<?php _e( 'Show sharing buttons.' , 'pojo-sharing'); ?>
		</label>
...
?>
<tr valign="top">
	<th scope="row"><label for="disable_css"><?php _e( 'Disable CSS and JS', 'pojo-sharing' ); ?></label></th>
	<td>
		<input id="disable_css" type="checkbox" name="disable_resources" <?php if ( $disable == 1 ) echo ' checked="checked"'; ?>/>  <small><em><?php _e( 'Advanced.  If this option is checked, you must include these files in your theme manually for the sharing links to work.', 'pojo-sharing' ); ?></em></small>
	</td>
</tr>
<?php
}
