...
class Sharing_Admin {
	...
	public function sharing_head() {
		wp_enqueue_script( 'sharing-js', WP_SHARING_PLUGIN_URL.'assets/js/admin-sharing.js', array( 'jquery-ui-draggable', 'jquery-ui-droppable', 'jquery-ui-sortable', 'jquery-form' ), 2 );
		wp_enqueue_style( 'sharing-admin', WP_SHARING_PLUGIN_URL.'assets/css/admin-sharing.css', false );
		wp_enqueue_style( 'sharing', WP_SHARING_PLUGIN_URL.'assets/css/sharing.css', false );
		wp_enqueue_script( 'sharing-js-fe', WP_SHARING_PLUGIN_URL . 'assets/js/sharing.js', array( ), 3 );
...
<?php
	...
	public function management_page() {
		...
		$shows = array_diff( $shows, array( 'attachment' ) );
		
		if ( false == function_exists( 'mb_stripos' ) ) {
			echo '<div id="message" class="updated fade"><h3>' . __( 'Warning! Multibyte support missing!', 'pojo-sharing' ) . '</h3>';
			echo "<p>" . sprintf( __( 'This plugin will work without it, but multibyte support is used <a href="%s">if available</a>. You may see minor problems with Tweets and other sharing services.', 'pojo-sharing' ), "http://www.php.net/manual/en/mbstring.installation.php" ) . '</p></div>';
		}

		if ( isset( $_GET['update'] ) && $_GET['update'] == 'saved' )
			echo '<div class="updated"><p>'.__( 'Settings have been saved', 'pojo-sharing' ).'</p></div>';
?>

	<div class="wrap">
	  	<div class="icon32" id="icon-options-general"><br /></div>
	  	<h2><?php _e( 'Sharing Settings', 'pojo-sharing' ); ?></h2>
		...
		<div class="share_manage_options">
	  	<h3><?php _e( 'Sharing Buttons', 'pojo-sharing' ) ?></h3>
	  	<p><?php _e( 'Add sharing buttons to your blog and allow your visitors to share posts with their friends.', 'pojo-sharing' ) ?></p>

	  	<div id="services-config">
	  		<table id="available-services">
					<tr>
		  			<td class="description">
		  				<h3><?php _e( 'Available Services', 'pojo-sharing' ); ?></h3>
		  				<p><?php _e( "Drag and drop the services you'd like to enable into the box below.", 'pojo-sharing' ); ?></p>
		  				<?php /*<p><a href="#TB_inline?height=395&amp;width=600&amp;inlineId=new-service" title="<?php echo esc_attr( __( 'Add a new service', 'pojo-sharing' ) ); ?>" class="thickbox" id="add-a-new-service"><?php _e( 'Add a new service', 'pojo-sharing' ); ?></a></p> */ ?>
		  			</td>
		  			<td class="services">
						...
						<?php
			  				if ( -1 == get_option( 'blog_public' ) )
								echo '<p><strong>'.__( 'Please note that your services have been restricted because your site is private.', 'pojo-sharing' ).'</strong></p>';
		  				?>
		  				<br class="clearing" />
		  			</td>
					</tr>
  			...
  			<table id="enabled-services">
  				<tr>
  					<td class="description">
						<h3>
							<?php _e( 'Enabled Services', 'pojo-sharing' ); ?>
							<img src="<?php echo admin_url( 'images/loading.gif' ); ?>" width="16" height="16" alt="loading" style="vertical-align: middle; display: none" />
						</h3>
						<p><?php _e( 'Services dragged here will appear individually.', 'pojo-sharing' ); ?></p>
  					</td>
	  				<td class="services" id="share-drop-target">
			  				<h2 id="drag-instructions" <?php if ( count( $enabled['visible'] ) > 0 ) echo ' style="display: none"'; ?>><?php _e( 'Drag and drop available services here.', 'pojo-sharing' ); ?></h2>
					...
					<td id="hidden-drop-target" class="services">
			  				<p><?php _e( 'Services dragged here will be hidden behind a share button.', 'pojo-sharing' ); ?></p>
			...
			<table id="live-preview">
				<tr>
					<td class="description">
						<h3><?php _e( 'Live Preview', 'pojo-sharing' ); ?></h3>
					</td>
					<td class="services">
						<h2<?php if ( count( $enabled['all'] ) > 0 ) echo ' style="display: none"'; ?>><?php _e( 'Sharing is off. Add services above to enable.', 'pojo-sharing' ); ?></h2>
						<div class="sharedaddy sd-sharing-enabled">
							...
							<div class="sd-content">
								<ul class="preview">
					                ...
					                <li class="advanced"><a href="#" class="sharing-anchor sd-button share-more"><span><?php _e( 'More', 'pojo-sharing' ); ?></span></a></li>
					                <?php endif; ?>
					            </ul>
	  	...
	  	<form method="post" action="">
	  		<table class="form-table">
	  			<tbody>
	  				<tr valign="top">
	  					<th scope="row"><label><?php _e( 'Button style', 'pojo-sharing' ); ?></label></th>
	  					<td>
	  						<select name="button_style" id="button_style">
	  							<option<?php if ( $global['button_style'] == 'icon-text' ) echo ' selected="selected"';?> value="icon-text"><?php _e( 'Icon + text', 'pojo-sharing' ); ?></option>
	  							<option<?php if ( $global['button_style'] == 'icon' ) echo ' selected="selected"';?> value="icon"><?php _e( 'Icon only', 'pojo-sharing' ); ?></option>
	  							<option<?php if ( $global['button_style'] == 'text' ) echo ' selected="selected"';?> value="text"><?php _e( 'Text only', 'pojo-sharing' ); ?></option>
	  							<option<?php if ( $global['button_style'] == 'official' ) echo ' selected="selected"';?> value="official"><?php _e( 'Official buttons', 'pojo-sharing' ); ?></option>
	  						</select>
	  					</td>
	  				</tr>
	  				<tr valign="top">
	  					<th scope="row"><label><?php _e( 'Sharing label', 'pojo-sharing' ); ?></label></th>
	  					<td>
	  						<input type="text" name="sharing_label" value="<?php echo esc_attr( $global['sharing_label'] ); ?>" />
	  					</td>
	  				</tr>
	  				<tr valign="top">
	  					<th scope="row"><label><?php _e( 'Open links in', 'pojo-sharing' ); ?></label></th>
	  					<td>
	  						<select name="open_links">
	  							<option<?php if ( $global['open_links'] == 'new' ) echo ' selected="selected"';?> value="new"><?php _e( 'New window', 'pojo-sharing' ); ?></option>
	  							<option<?php if ( $global['open_links'] == 'same' ) echo ' selected="selected"';?> value="same"><?php _e( 'Same window', 'pojo-sharing' ); ?></option>
	  						</select>
	  					</td>
	  				</tr>
	  				<?php echo apply_filters( 'sharing_show_buttons_on_row_start', '<tr valign="top">' ); ?>
	  					<th scope="row"><label><?php _e( 'Show buttons on', 'pojo-sharing' ); ?></label></th>
	  					<td>
						<?php
							$br = false;
		  	...
		  	<p class="submit">
					<input type="submit" name="submit" class="button-primary" value="<?php _e( 'Save Changes', 'pojo-sharing' ); ?>" />
				</p>
	  ...
	  <div id="new-service" style="display: none">
	  	<form method="post" action="<?php echo admin_url( 'admin-ajax.php' ); ?>" id="new-service-form">
	  		<table class="form-table">
	  			<tbody>
	  				<tr valign="top">
	  					<th scope="row" width="100"><label><?php _e( 'Service name', 'pojo-sharing' ); ?></label></th>
	  					<td>
	  						<input type="text" name="sharing_name" id="new_sharing_name" size="40" />
	  					</td>
	  				</tr>
	  				<tr valign="top">
	  					<th scope="row" width="100"><label><?php _e( 'Sharing URL', 'pojo-sharing' ); ?></label></th>
	  					<td>
	  						<input type="text" name="sharing_url" id="new_sharing_url" size="40" />

	  						<p><?php _e( 'You can add the following variables to your service sharing URL:', 'pojo-sharing' ); ?><br/>
	  						<code>%post_title%</code>, <code>%post_url%</code>, <code>%post_full_url%</code>, <code>%post_excerpt%</code>, <code>%post_tags%</code></p>
	  					</td>
	  				</tr>
	  				<tr valign="top">
	  					<th scope="row" width="100"><label><?php _e( 'Icon URL', 'pojo-sharing' ); ?></label></th>
	  					<td>
	  						<input type="text" name="sharing_icon" id="new_sharing_icon" size="40" />
	  						<p><?php _e( 'Enter the URL of a 16x16px icon you want to use for this service.', 'pojo-sharing' ); ?></p>
	  					</td>
	  				</tr>
	  				<tr valign="top" width="100">
	  					<th scope="row"></th>
	  					<td>
								<input type="submit" class="button-primary" value="<?php _e( 'Create Share Button', 'pojo-sharing' ); ?>" />
	  						<img src="<?php echo admin_url( 'images/loading.gif' ); ?>" width="16" height="16" alt="loading" style="vertical-align: middle; display: none" />
	  					</td>
	  				</tr>
		...
		<?php do_action( 'post_admin_screen_sharing' ) ?>
				...
				<div class="inerror" style="display: none; margin-top: 15px">
					<p><?php _e( 'An error occurred creating your new sharing service - please check you gave valid details.', 'pojo-sharing' ); ?></p>
				</div>
