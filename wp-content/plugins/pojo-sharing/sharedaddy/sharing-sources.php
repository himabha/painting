...
class Share_Email extends Sharing_Source {
	...
	public function get_name() {
		return _x( 'Email', 'as sharing source', 'pojo-sharing' );
	}
...
?>
<div class="response">
	<div class="response-title"><?php _e( 'This post has been shared!', 'pojo-sharing' ); ?></div>
 	<div class="response-sub"><?php printf( __( 'You have shared this post with %s', 'pojo-sharing' ), esc_html( $target_email ) ); ?></div>
 	<div class="response-close"><a href="#" class="sharing_cancel"><?php _e( 'Close', 'pojo-sharing' ); ?></a></div>
</div>
<?php
				}
				else
	...
	public function get_display( $post ) {
		return $this->get_link( get_permalink( $post->ID ), _x( 'Email', 'share to', 'pojo-sharing' ), __( 'Click to email this to a friend', 'pojo-sharing' ), 'share=email' );
	}
...
?>
	<div id="sharing_email" style="display: none;">
		<form action="<?php echo esc_url( $_SERVER['REQUEST_URI'] ); ?>" method="post">
			<label for="target_email"><?php _e( 'Send to Email Address', 'pojo-sharing' ) ?></label>
			...
			<?php else : ?>
				...
				<label for="source_name"><?php _e( 'Your Name', 'pojo-sharing' ) ?></label>
				<input type="text" name="source_name" id="source_name" value="" />

				<label for="source_email"><?php _e( 'Your Email Address', 'pojo-sharing' ) ?></label>
				<input type="text" name="source_email" id="source_email" value="" />

			<?php endif; ?>

			<?php do_action( 'sharing_email_dialog', 'pojo-sharing' ); ?>

			<img style="float: right; display: none" class="loading" src="<?php echo apply_filters( 'pojo_static_url', plugin_dir_url( __FILE__ ) . 'assets/images/loading.gif' ); ?>" alt="loading" width="16" height="16" />
			<input type="submit" value="<?php _e( 'Send Email', 'pojo-sharing' ); ?>" class="button sharing_send" />
			<a href="#cancel" class="sharing_cancel"><?php _e( 'Cancel', 'pojo-sharing' ); ?></a>

			<div class="errors errors-1" style="display: none;">
				<?php _e( 'Post was not sent - check your email addresses!', 'pojo-sharing' ); ?>
			</div>

			<div class="errors errors-2" style="display: none;">
				<?php _e( 'Email check failed, please try again', 'pojo-sharing' ); ?>
			</div>

			<div class="errors errors-3" style="display: none;">
				<?php _e( 'Sorry, your blog cannot share posts by email.', 'pojo-sharing' ); ?>
			</div>
		</form>
	</div>
<?php
...
class Share_Twitter extends Sharing_Source {
	...
	public function get_name() {
		return __( 'Twitter', 'pojo-sharing' );
	}

	function sharing_twitter_via( $post ) {
		// Allow themes to customize the via
		return apply_filters( 'pojo_sharing_twitter_via', '', $post->ID );
	}

	public function get_related_accounts( $post ) {
		// Format is 'username' => 'Optional description'
		$related_accounts = apply_filters( 'pojo_sharing_twitter_related', array(), $post->ID );
	...
	public function get_display( $post ) {
		...
		if ( $this->smart ) {
			...
			return $this->get_link( get_permalink( $post->ID ), _x( 'Twitter', 'share to', 'pojo-sharing' ), __( 'Click to share on Twitter', 'pojo-sharing' ), 'share=twitter', 'sharing-twitter-' . $post->ID );
		}
	}
...
class Share_Stumbleupon extends Sharing_Source {
	...
	public function get_name() {
		return __( 'StumbleUpon', 'pojo-sharing' );
	}
	...
	public function get_display( $post ) {
		if ( $this->smart )
			...
			return $this->get_link( get_permalink( $post->ID ), _x( 'StumbleUpon', 'share to', 'pojo-sharing' ), __( 'Click to share on StumbleUpon', 'pojo-sharing' ), 'share=stumbleupon' );
	}
...
class Share_Reddit extends Sharing_Source {
	...
	public function get_display( $post ) {
		if ( $this->smart )
			...
			return $this->get_link( get_permalink( $post->ID ), _x( 'Reddit', 'share to', 'pojo-sharing' ), __( 'Click to share on Reddit', 'pojo-sharing' ), 'share=reddit' ); // Edited by Anas H. Sulaiman
	}
...
class Share_Digg extends Sharing_Source {
	...
	public function get_display( $post ) {
		if ( $this->smart ) {
			$url = $this->get_link( 'http://digg.com/submit?url='. rawurlencode( $this->get_share_url( $post->ID ) ) . '&amp;title=' . rawurlencode( $post->post_title ), 'Digg', __( 'Click to Digg this post', 'pojo-sharing' ) );
			return '<div class="digg_button">' . str_replace( 'class="', 'class="DiggThisButton DiggCompact ', $url ) . '</div>';
		} else {
			return $this->get_link( get_permalink( $post->ID ), _x( 'Digg', 'share to', 'pojo-sharing' ), __( 'Click to Digg this post', 'pojo-sharing' ), 'share=digg' );
		}
	}
...
class Share_LinkedIn extends Sharing_Source {
	...
	public function get_name() {
		return __( 'LinkedIn', 'pojo-sharing' );
	}
	...
	public function get_display( $post ) {
		...
		if ( $this->smart )
			...
			$display = $this->get_link( get_permalink( $post->ID ), _x( 'LinkedIn', 'share to', 'pojo-sharing' ), __( 'Click to share on LinkedIn', 'pojo-sharing' ), 'share=linkedin', 'sharing-linkedin-' . $post->ID );
...
class Share_Facebook extends Sharing_Source {
	...
	public function get_name() {
		return __( 'Facebook', 'pojo-sharing' );
	}
	...
	function guess_locale_from_lang( $lang ) {
		...
		if ( !class_exists( 'GP_Locales' ) ) {
			if ( !defined( 'POJO__GLOTPRESS_LOCALES_PATH' ) || !file_exists( POJO__GLOTPRESS_LOCALES_PATH ) ) {
				return false;
			}

			require POJO__GLOTPRESS_LOCALES_PATH;
		}
	...
	public function get_display( $post ) {
		...
		if ( 'icon-text' == $this->button_style || 'text' == $this->button_style )
			sharing_register_post_for_share_counts( $post->ID );
		return $this->get_link( get_permalink( $post->ID ), _x( 'Facebook', 'share to', 'pojo-sharing' ), __( 'Share on Facebook', 'pojo-sharing' ), 'share=facebook', 'sharing-facebook-' . $post->ID );
	}
...
class Share_Print extends Sharing_Source {
	...
	public function get_name() {
		return __( 'Print', 'pojo-sharing' );
	}

	public function get_display( $post ) {
		return $this->get_link( get_permalink( $post->ID ) . ( ( is_single() || is_page() ) ? '#print': '' ), _x( 'Print', 'share to', 'pojo-sharing' ), __( 'Click to print', 'pojo-sharing' ) );
	}
}
...
class Share_PressThis extends Sharing_Source {
	...
	public function get_name() {
		return __( 'Press This', 'pojo-sharing' );
	}
	...
	public function get_display( $post ) {
		return $this->get_link( get_permalink( $post->ID ), _x( 'Press This', 'share to', 'pojo-sharing' ), __( 'Click to Press This!', 'pojo-sharing' ), 'share=press-this' );
	}
}
...
class Share_GooglePlus1 extends Sharing_Source {
	...
	public function get_display( $post ) {
		...
		if ( $this->smart ) {
			...
			return $this->get_link( get_permalink( $post->ID ), _x( 'Google', 'share to', 'pojo-sharing' ), __( 'Click to share on Google+', 'pojo-sharing' ), 'share=google-plus-1', 'sharing-google-' . $post->ID );
		}
	}
...
class Share_Custom extends Sharing_Advanced_Source {
	...
	public function get_display( $post ) {
		$str = $this->get_link( get_permalink( $post->ID ), esc_html( $this->name ), __( 'Click to share', 'pojo-sharing' ), 'share='.$this->id );
	...
	<table class="form-table">
		<tbody>
			...
			<tr>
				<th scope="row"></th>
				<td>
					<input class="button-secondary" type="submit" value="<?php _e( 'Save', 'pojo-sharing' ); ?>" />
					<a href="#" class="remove"><small><?php _e( 'Remove Service', 'pojo-sharing' ); ?></small></a>
				</td>
			</tr>
		</tbody>
	</table>
...
class Share_Tumblr extends Sharing_Source {
	...
	public function get_display( $post ) {
		if ( $this->smart ) {
			...
			return $this->get_link( get_permalink( $post->ID ), _x( 'Tumblr', 'share to', 'pojo-sharing' ), __( 'Click to share on Tumblr', 'pojo-sharing' ), 'share=tumblr' );
		}
	}
...
class Share_Pinterest extends Sharing_Source {
	...
	public function get_post_image( $content ) {
		...
		if ( function_exists('has_post_thumbnail') && has_post_thumbnail() ) {
			...
			// but... see http://wordpress.org/support/topic/pojo-youtube-embeds
			if ( ! $thumb ) return '';
	...
	public function get_display( $post ) {
		if ( $this->smart )
			return '<div class="pinterest_button"><a href="' . esc_url( 'http://pinterest.com/pin/create/button/?url='. rawurlencode( $this->get_share_url( $post->ID ) ) . '&description=' . rawurlencode( $post->post_title ) . '&media=' . rawurlencode( esc_url_raw( $this->get_post_image( $post ) ) ) ) . '" class="pin-it-button" count-layout="horizontal"> '. __( 'Pin It', 'pojo-sharing') .'</a></div>';
		else
			return $this->get_link( get_permalink( $post->ID ), _x( 'Pinterest', 'share to', 'pojo-sharing' ), __( 'Click to share on Pinterest', 'pojo-sharing' ), 'share=pinterest' );
	}
...
class Share_Pocket extends Sharing_Source {
	...
	public function get_display( $post ) {
		if ( $this->smart ) {
			...
			$button .= sprintf( '<a href="https://getpocket.com/save" class="pocket-btn" data-lang="%s" data-save-url="%s" data-pocket-count="%s" >%s</a>', 'en', esc_attr( $this->get_share_url( $post->ID ) ), $post_count, esc_attr__( 'Pocket', 'pojo-sharing' ) );
			$button .= '</div>';

			return $button;
		} else {
			return $this->get_link( get_permalink( $post->ID ), _x( 'Pocket', 'share to', 'pojo-sharing' ), __( 'Click to share on Pocket', 'pojo-sharing' ), 'share=pocket' );
		}
	...
	function display_footer() {
		...
		function pojo_sharing_pocket_init() {
			jQuery.getScript( 'https://widgets.getpocket.com/v1/j/btn.js?v=1' );
		}
		jQuery( document ).on( 'ready', pojo_sharing_pocket_init );
		jQuery( document.body ).on( 'post-load', pojo_sharing_pocket_init );
		</script>
		<?php
		else :
			$this->js_dialog( $this->shortname, array( 'width' => 450, 'height' => 450 ) );
