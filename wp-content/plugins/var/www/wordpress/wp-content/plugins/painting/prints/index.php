...
<form action="<?php echo $_SERVER['PHP_SELF']; ?>?page=prints" name="print_form1" id="print_form1" method="POST" enctype="multipart/form-data">
	<div class="form-group">
		...
		<span class="img_loader"><img src="<?php echo dirname(dirname(__FILE__)).'/ajax-loader.gif';?>"></span>
	</div>
...
<form class="print_form2" id="print_form2" name="print_form2">
<div class="form-group">
	<?php
    ...
    if (isset($prints) && !empty($prints)) {
		...
		<?php
            foreach ($prints as $index => $print) {
                $print = str_replace("_", " ", ucfirst($index));
                $options .= '<option value="'.$index.'">'.$print.'</option>';
            }
        echo $options; ?>
			</select>
	<?php
	...
	<div class="form-group thickness_width">
		...
		<input name="print_width" class="print_size" id="print_width" type="text" min="0" max="120" onkeyup="onsizekeyup(this);"/><?php echo $helper->getHebrewText('cm');?>
	...
	<div class="form-group thickness_height">
		...
		<input name="print_height" class="print_size" id="print_height" type="text" min="0" max="140" onkeyup="onsizekeyup(this);"/><?php echo $helper->getHebrewText('cm');?>
	...
	<div class="form-group laminate_dropdown">
		<label for='print_laminate_select'><?php echo $helper->getHebrewText('choose_lamination');?></label>
		<select id='print_laminate_select' class='print_laminate' onchange='calc()' name='print_laminate_select'>
		</select>
	</div>
	...
	<div class="form-group frame_required">
		<label><input type="checkbox" name="frame_required" id="frame_required" value="" onclick="onframerequired(this)"; /> <?php echo $helper->getHebrewText('with_frame');?></label>
	</div>
	...
	<div class="form-group field_cover">
	  ...
	  <div class="form-group field_extra_thickness">
		<label for="frame_colors"><?php echo $helper->getHebrewText('select_extra_width');?>: </label>
			<input id="frame_extra_thickness" min="2" max="10" value="2" type="number" name="frame_extra_thickness" onchange="onquantitychange(this)" onkeyup="onextrathicknesschange()"/><?php echo $helper->getHebrewText('cm');?>cm
		</label>
	  </div>
		<!--
	...
	<div class="form-group field_quantity">
		...
		<input id="print_quantity" min="1" value="1" type="number" name="print_quantity" onchange="onquantitychange(this)" onkeyup="onquantitychange(this)"/>
	</div>
...
<div class="right_section">
	<?php include($_SERVER['DOCUMENT_ROOT'].ROOT_DIRECTORY.'/frame-grid.php'); ?>
</div>
<script>
...
function calc(){
	...
	var print_laminate_select = $("#print_laminate_select.laminate_enabled").val();
	var print_laminate_check = $("#print_laminate_check.laminate_enabled").is(":checked");
	var print_view = $(".print_laminate.hanging_type_enabled").val();
		...
		//if(print_width/r1 == image_width/r2 && print_height/r1 == image_height/r2)
		{
			...
			$.getJSON('<?php echo plugins_url('types.json', dirname(__FILE__));?>', function(data){
				if(typeof data.categories.prints[page] != 'undefined')
				{
					...
					{
						max_dimensions[0] =  data.categories.prints[page].sizes.max_size_one_side;
						max_dimensions[1] =  data.categories.prints[page].sizes.max_size_one_side;
					}
					else
					{
					...
					if(print_width <= max_dimensions[0] && print_height <= max_dimensions[1])
					{
						...
						if(typeof print_finishing_select != 'undefined' && print_finishing_select !="")
						{
							...
							$(data.categories.prints[page].finishing).each(function(index){
								var finishing = this;
								if(print_finishing == finishing.type)
								{
									//if(page == 'paper')
									{
						...
						if(typeof print_laminate_select != 'undefined' && print_laminate_select !="")
						{
							var find = "_";
							var re = new RegExp(find, 'g');
							var print_laminate = print_laminate_select.replace(re, " ");
							$(data.categories.prints[page].laminate).each(function(index){
								var laminate = this;
								if(print_laminate == laminate.type)
								{
					...
					{
						$("#print_size_error").html("Width or height should be less than "+data.categories.prints[page].sizes.max_size_one_side+" cm");
						$("#price").html("");
					}
				}
			});
					...
					if(typeof frame_extra != 'undefined' && frame_extra !="")
					{
						...
						$(data.categories.prints[page].extras).each(function(index){
							if(frame_extra != "Without" && (typeof this.max_dimensions != "undefined" && maxdimension <= Math.max.apply(null, [this.max_dimensions[0], this.max_dimensions[1]]) && mindimension <= Math.min.apply(null, [this.max_dimensions[0], this.max_dimensions[1]])))
							{
								...
								{
									price = 0;
									$("#frame_size_error").html("Size should be maximum "+this.max_dimensions[0]+"x"+this.max_dimensions[1]);
									return false;
								}
...
function onsizekeyup(elem)
{
	var image_width = $("#image_width").val();
	var image_height = $("#image_height").val();
	var width = $(elem).val();
	var r = gcd(image_width, image_height);
	var high = image_width/r;
	var low = image_height/r;
	if(width != ""){
		if(high > low)
		{
			var val = high/low;
		...
		if((val*width).toString().indexOf(".") !== -1)
		{
			var height = $(".print_size").not(elem).val(parseFloat(val*width).toFixed(2));
		}
		else
		{
			var height = $(".print_size").not(elem).val(parseInt(val*width));
		}
	}
	calc();
}
			...
			if(typeof content.laminate != "undefined")
			{
				if(content.laminate.length > 1)
				{
					...
					$(content.laminate).each(function(index){
						...
						laminate += "<option value="+type+">"+this.type+"</option>";
					});
					$(".print_laminate").removeClass("laminate_enabled");
					$(".print_laminate:eq(0)").addClass("laminate_enabled");
					//$(".print_laminate:eq(0) option").not($(".print_laminate:eq(0) option:eq(0)")).remove();
					$(".print_laminate:eq(0) option").remove();
					$(".print_laminate:eq(0) option:eq(0)").attr("selected", true);
					$(".print_laminate").append(laminate);
				}
				else{
					$(".laminate_line").css("display", "block");
					$(".laminate_dropdown").css("display", "none");
					$(".print_laminate").removeClass("laminate_enabled");
					$(".print_laminate:eq(1)").addClass("laminate_enabled");
					$(".print_laminate:eq(1)").val(content.laminate[0].type);
					$("#laminate_content").html(content.laminate[0].type);
				}
			}
			else{
				$(".print_laminate").removeClass("laminate_enabled");
			...
			$.ajax({
				...
				success: function(res)
				{
					...
					if(typeof res.error == "undefined")
					{
						...
						$("#image_content").html("<img src='"+res.url+"' height='200' alt='"+res.filename+"'>");
			...
			$.getJSON('<?php echo plugins_url('types.json', dirname(__FILE__));?>', function(data){
				...
				if(val == 'paper')
				{
					...
					$(content.types).each(function(index){
						...
						var re = new RegExp(find, 'g');
						var type = this.type.replace(re, "_");
						papertypes += "<option value="+type+">"+this.type+"</option>";
					});
					...
					$(content.extras).each(function(index){
						...
						var re = new RegExp(find, 'g');
						var type = this.type.replace(re, "_");
						extras += "<option value="+type.toLowerCase()+">"+this.type+"</option>";
					});
				...
				if(typeof content.thickness_by_mm != "undefined")
				{
					if(content.thickness_by_mm.length > 1)
					{
						...
						$(content.thickness_by_mm).each(function(index, thickness){
							thickness_by_mm += "<option value="+index+">"+thickness+"mm</option>";
						});
