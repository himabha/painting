...
<div class="form-group">
	...
	<div class="form-group thickness_width">
		<label for="mirror_width"><?php echo $helper->getHebrewText('select_other_products'); ?>Width: </label>
		<input name="mirror_width" class="mirror_size" id="mirror_width" type="text" min="0" max="120"/><?php echo $helper->getHebrewText('cm'); ?>
	</div>
	<div class="form-group thickness_height">
		<label for="mirror_height"><?php echo $helper->getHebrewText('height'); ?>:</label>
		<input name="mirror_height" class="mirror_size" id="mirror_height" type="text" min="0" max="140"/><?php echo $helper->getHebrewText('cm'); ?>
	</div>
	<div class="form-group">
		...
		if(typeof content != 'undefined' && content.is_catalogue == false)
		{
			$(content.sizes).each(function(index){
				...
				if(size.thickness_by_mm == thickness)
				{
					...
					{
						$("#mirror_size_error").html("Maximum size should be "+size.dimensions[0]+"X"+size.dimensions[1]);
						$("#price").html("");
						return false;
					}
					...
					if(typeof mirror_finishing != 'undefined')
					{
						...
						$(size.finishing).each(function(index){
							var finishing = this;
							if(checked == true && mirror_finishing == finishing.type && finishing.chose_frame == true)
							{
								frame_calc();
							}
						});
		...
		else if(typeof data.categories.mirrors[page] != 'undefined' && data.categories.mirrors[page].is_catalogue == true)
		{
			$(data.categories.mirrors[page].catalogues[2].sizes).each(function(index){
				...
				if(mirror_width < min_min_dimensions || mirror_height < min_min_dimensions || mirror_width > max_max_dimensions || mirror_height > max_max_dimensions)
				{
					...
					$("#mirror_size_error").html("Maximum size should be "+size.min_dimensions[0]+"X"+size.min_dimensions[1]+" and atleast " +size.max_dimensions[0]+"X"+size.max_dimensions[1]);
					$("#price").html("");
					return false;
				}
				...
				if(thickness == this.thickness_by_mm)
				{
					if(typeof this.finishing != "undefined")
					{
						if(this.finishing.length > 1)
						{
							...
							$(".mirror_finishing:eq(1)").val(this.finishing[0].type);
							$("#finishing_content").html(this.finishing[0].type);
						}
					}
					else{
						...
						if(content.sizes.length > 1)
						{
							...
							if(typeof content.sizes[0].finishing != "undefined")
							{
								if(content.sizes[0].finishing.length > 1)
								{
									...
									$("#finishing_content").html(content.sizes[0].finishing[0].type);
								}
							}
							else{
								$(".finishing_dropdown, .finishing_line").css("display", "none");
