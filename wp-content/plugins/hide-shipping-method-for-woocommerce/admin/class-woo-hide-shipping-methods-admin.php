...
class Woo_Hide_Shipping_Methods_Admin
{
                    ...
                    if ( in_array( $val->id, $whsm_cslist['whsm_fs'], true ) ) {
                        if ( 'yes' === $val->enabled ) {
                            ...
                            if ( !empty($val->instance_id) ) {
                                $fsmi = get_option( 'flexible_shipping_methods_' . $val->instance_id );
                                if ( !empty($fsmi) ) {
                                    foreach ( $fsmi as $fsmi_val ) {
                                        
                                        if ( 'advanced' === $args ) {
                                            if ( !empty($fsmi_val['method_free_shipping']) ) {
                                                $zone_status_array[$fsmi_val['id_for_shipping']] = $the_zone['zone_name'] . ' - ' . $fsmi_val['method_title'];
                                            }
                                        } else {
                                            $zone_status_array[$fsmi_val['id_for_shipping']] = $the_zone['zone_name'] . ' - ' . $fsmi_val['method_title'];
                                        }
    ...
    public function whsm_compatible_shipping_plugin_list()
    {
        ...
        $whsm_fs = array( 'flexible_shipping' );
        $whsm_wbs = array( 'wbs' );
        $retun_pram = array(
