...
var pojo_admin_customizer_controls = ( function( $, window ) {
	var _bodyListener = function( e ) {
		...
		init = function() {
			...
			$( 'a.pojo-btn-control-toggle-wrap' ).on( 'click', function( e ) {
				e.stopPropagation();
				_control_customizer_toggle_wrap( $( this ).next( 'div.customize-control-toggle-wrap' ) );
			} );

			$( 'div.accordion-container > #customize-theme-controls' ).on( 'click keydown', '.accordion-section-title', function( e ) {
			...
			$( 'div.pojo-customizer-upload-image-wrap' ).on( 'click', 'a.at-image-upload',function( e ) {
			...
			$.each( window._wpCustomizeSettings.controls, function( id, data ) {
