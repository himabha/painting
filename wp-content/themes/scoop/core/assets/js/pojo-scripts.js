	...
	var is_rtl = false,
		...
		initAnchorLinks = function() {
			$( 'a' ).on( 'click', function( e ) {
				...
				if ( ! isSameHostname || ! isSamePathname || ! hasHash ) {
					return;
				}
				
				var $targetElement = $( this.hash );
				if ( ! $targetElement.length || ! $targetElement.hasClass( 'pojo-menu-anchor' ) ) {
					return;
				}
				...
				if ( 1 <= $stickyHeader.length ) {
					offsetTop -= $stickyHeader.height();
				}
				
				if ( hasAdminBar ) {
					offsetTop -= adminBarSize;
				}
		...
		initBackgroundVideo = function() {
			...
			window.onYouTubeIframeAPIReady = function() {
				$( 'div.pb-youtube-frame' ).each( function() {
					var $video = $( this ),
						params = $video.data(),
