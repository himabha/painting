	...
	public function widget( $args, $instance ) {
		...
		foreach ( self::get_social_available() as $social ) {
			...
			if ( ! empty( $instance[ $field_key ] ) ) {
				...
				$items_array[] = '<li class="social-' . self::get_social_slug( $social ) . '"><a href="' . esc_attr( $instance[ $field_key ] ) . '"'. $tooltip_attrs . ' title="' . esc_attr__( $social, 'pojo' ) . '" target="_blank">' . $icon . '</a></li>';
			}
		}
