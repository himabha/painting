...
function pojo_enqueue_scripts() {
	if ( ! is_admin() ) {
		// Google CDN for jQuery with fallback to local if offline.
		wp_deregister_script( 'jquery' );
		wp_register_script( 'jquery', '//ajax.googleapis.com/ajax/libs/jquery/1.11.1/jquery.min.js', array(), null, false );
		add_filter( 'script_loader_src', 'pojo_jquery_local_fallback', 10, 2 );
	}
	...
	if ( POJO_DEVELOPER_MODE ) {
		...
		// imgLiquid
		wp_register_script( 'imgLiquid', get_template_directory_uri() . '/core/assets/js/jquery.imgLiquid.js', array( 'jquery' ), '1.5.25', true );
		...
		wp_register_script( 'bxslider', get_template_directory_uri() . '/core/assets/bxslider/jquery.bxslider.min.js', array( 'jquery', 'fitvids' ), '4.1.1' );
		...
		wp_register_script( 'pojo-scripts', get_template_directory_uri() . '/core/assets/js/pojo-scripts.js', array( 'jquery', 'imgLiquid' ), POJO_CORE_VERSION, true );
	...
	wp_enqueue_script( 'masterslider', get_template_directory_uri() . '/core/assets/masterslider/masterslider.min.js', array( 'jquery' ), '2.9.5', true );
...
function pojo_jquery_local_fallback( $src, $handle ) {
	static $add_jquery_fallback = false;

	if ( $add_jquery_fallback ) {
		echo '<script>window.jQuery || document.write(\'<script src="' . get_template_directory_uri() . '/core/assets/js/jquery-1.11.1.min.js"><\/script>\')</script>' . "\n";
